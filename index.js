...
app.get("/movies/:id", async function (request, response) {
  ...
  movie?
  response.send(movie):response.status(404).send({message:"No such file found"})
});
...
app.delete("/movies/:id",async function(request,response){
  const { id } = request.params;
  const delRes=await client.db("b30wd").collection("movies").deleteOne({id:id})
  response.send(delRes);
});
app.put("/movies/:id", async function (request, response) {
  const { id } = request.params;
  const data = request.body;
  const result = await client.db("b30wd").collection("movies").updateOne({id:id},{$set:data});
  response.send(result);
});